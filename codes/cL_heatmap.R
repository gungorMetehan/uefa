# importing data
uefa_cL <- read.csv("https://raw.githubusercontent.com/gungorMetehan/uefa/refs/heads/main/datasets/uefa_cL.csv", header = T, sep = ",")
...
uefa_cL$season <- as.factor(uefa_cL$season)
season_levels <- levels(uefa_cL$season)
...
ggplot(uefa_cL, aes(x = season, y = country, fill = as.factor(highest))) +
  ...
  scale_fill_manual(
    values = c("0" = "grey95", "1" = "#8567a6", "2" = "#c297d2", "3" = "#e0b1e8", "4" = "#ffcbff"),
